...
const LandmarkCard = ({ landmark }) => {
  ...
  useEffect(() => {
    // Check if landmark is in collection when component mounts
    const checkStatus = async () => {
      if (collectionService.isAuthenticated() && landmark.id) {
        try {
          const saved = await collectionService.checkCollectionStatus(landmark.id);
          setIsSaved(saved);
        } catch (error) {
  ...
  const handleHeartClick = async (e) => {
    ...
    if (!collectionService.isAuthenticated()) {
      alert('Please log in to save landmarks to your collection.');
      return;
    }

    setIsLoading(true);
    try {
      const result = await collectionService.toggleCollection(landmark.id, 'landmark');
